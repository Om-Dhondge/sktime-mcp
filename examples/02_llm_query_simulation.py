...
def simulate_query_1():
    ...
    print_llm_thought("User needs forecasting with: 1) missing data handling, 2) prediction intervals")
    
    # Step 2: Search for capability tags
    print_llm_thought("Let me find estimators with prediction interval capability...")
    
    print_tool_call("list_estimators", {
        "task": "forecasting",
    ...
    if result["success"] and result["estimators"]:
        ...
        if desc["success"]:
            print(f"   It supports prediction intervals (capability:pred_int = True)")
...
def simulate_query_2():
    ...
    print_result({"name": desc1.get("name"), "task": desc1.get("task"), "success": desc1["success"]})
    ...
    print_result({"name": desc2.get("name"), "task": desc2.get("task"), "success": desc2["success"]})
    
    if desc1["success"] and desc2["success"]:
        # Step 4: Instantiate both
        print_llm_thought("Both are valid forecasters. Let me run them...")
        
        print_tool_call("instantiate_estimator", {"estimator": "NaiveForecaster", "params": {"sp": 11}})
        ...
        if h1:
            print_tool_call("fit_predict", {"estimator_handle": h1, "dataset": "sunspots", "horizon": 6})
            pred1 = fit_predict_tool(h1, "sunspots", 6)
            print_result({"success": pred1["success"], "horizon": pred1.get("horizon")})
        
        if h2:
            print_tool_call("fit_predict", {"estimator_handle": h2, "dataset": "sunspots", "horizon": 6})
        ...
        if h1 and pred1["success"]:
            print(f"   - NaiveForecaster predictions generated successfully")
        if h2 and pred2["success"]:
            print(f"   - ThetaForecaster predictions generated successfully")
...
def simulate_query_3():
    ...
    if result.valid:
        ...
        print(f"   ❌ This pipeline has issues:")
